...
vi.mock('@/features/auth/stores/auth-store', () => ({
  useAuthStore: Object.assign(
    (selector?: any) => {
      const state = {
        user: { id: 'u1', name: 'John', email: 'john@test.com', avatar: 'JD', role: 'USER' },
        isLoading: false,
      };
...
vi.mock('@/features/kanban/api/mock-api', () => ({
  ...
  createBoard: vi
    .fn()
    .mockResolvedValue({ ok: true, data: { id: 'b-new', name: 'New Board', lists: [], members: [] } }),
}));

vi.mock('@/features/kanban/components/new-board-modal', () => ({
  NewBoardModal: ({ open, onClose, onCreate }: any) =>
    open ? (
