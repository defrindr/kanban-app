...
import { fetchMyTasks, fetchBoards } from '@/features/kanban/api/mock-api'
        ...
        {loading ? (
          ...
          <div className="space-y-8">
            {Array.from(grouped.entries()).sort().map(([bid, bCards]) => (
              <div key={bid}>
                <Link href={`/board/${bid}`} className="inline-flex items-center gap-2 text-lg font-semibold text-gray-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 transition-colors mb-3">
                  {boardNames.get(bid) || 'Unknown Board'}
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" /></svg>
                </Link>
                <div className="space-y-2">
                  {bCards.map((c) => (
                    <Link key={c.id} href={`/board/${bid}`} className="block bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700/50 rounded-xl p-4 hover:border-blue-500/30 hover:shadow-md transition-all duration-200">
