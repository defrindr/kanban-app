    ...
    <div className="w-80 h-full bg-white dark:bg-gray-900 border-l border-gray-200 dark:border-gray-800 flex flex-col">
      <div className="border-b border-gray-200 dark:border-gray-800 flex-shrink-0">
        <div className="flex text-xs">
          <button onClick={() => onTabChange('settings')} className={`flex-1 px-3 py-2.5 text-xs font-medium transition-colors border-b-2 ${activeTab === 'settings' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300'}`}>Settings</button>
          <button onClick={() => onTabChange('webhooks')} className={`flex-1 px-3 py-2.5 text-xs font-medium transition-colors border-b-2 ${activeTab === 'webhooks' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300'}`}>Webhooks</button>
          <button onClick={() => onTabChange('activity')} className={`flex-1 px-3 py-2.5 text-xs font-medium transition-colors border-b-2 ${activeTab === 'activity' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300'}`}>Activity</button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {activeTab === 'activity' ? (
          ...
          <div className="space-y-4">
            <div>
              <h4 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2.5">Webhooks ({webhooks.length})</h4>
              <div className="space-y-2 mb-4">
                {webhooks.length === 0 ? (
                  <p className="text-xs text-gray-400 dark:text-gray-500 text-center py-4">No webhooks configured</p>
                ) : (
                  webhooks.map((webhook) => (
                    <div key={webhook.id} className="p-2.5 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 group">
                        ...
                        <div className="flex-1 min-w-0 flex-1">
                          ...
                          <div className="flex flex-wrap gap-1 mt-1.5">
                            {webhook.events.map((evt) => (
                              <span key={evt} className="inline-block text-[10px] px-1.5 py-0.5 bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 rounded">{evt}</span>
                            ))}
                          </div>
                          <div className="text-xs text-gray-500 dark:text-gray-400 mt-1.5">
                            <label className="flex items-center gap-1 cursor-pointer">
                              <input type="checkbox" checked={webhook.active} onChange={(e) => onUpdateWebhook?.(webhook.id, { active: e.target.checked })} className="w-3 h-3" />
                              <span>Active</span>
                            </label>
                          </div>
                        </div>
                        <button onClick={() => onDeleteWebhook?.(webhook.id)} className="opacity-0 group-hover:opacity-100 text-gray-400 hover:text-red-500 transition-all p-0.5 flex-shrink-0">
            ...
            <div className="pt-3 border-t border-gray-200 dark:border-gray-800">
              <h5 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2.5">Add Webhook</h5>
              <div className="space-y-2">
                <input type="url" value={newWebhookUrl} onChange={(e) => setNewWebhookUrl(e.target.value)} placeholder="Webhook URL"
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {['card:created', 'card:updated', 'card:deleted', 'card:moved', 'list:created', 'list:updated', 'list:deleted', 'comment:created', 'comment:updated', 'comment:deleted'].map((evt) => (
                      <label key={evt} className="flex items-center gap-2 text-xs text-gray-700 dark:text-gray-300 cursor-pointer">
                        <input type="checkbox" checked={newWebhookEvents.includes(evt)} onChange={(e) => {
                          setNewWebhookEvents(e.target.checked ? [...newWebhookEvents, evt] : newWebhookEvents.filter(x => x !== evt))
                        }} className="w-3 h-3" />
                        <span>{evt}</span>
                      </label>
                    ))}
                ...
                <button onClick={() => {
                  ...
                  className="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium rounded-lg transition-colors disabled:opacity-50">Create Webhook</button>
              </div>
            </div>
          </div>
        ) : (
