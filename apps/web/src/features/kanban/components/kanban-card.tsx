  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners}>
      <div
        ...
        `}
      >
        {card.coverColor && <div className={`h-2 ${coverColors[card.coverColor] || ''}`} />}
        ...
        <div className="p-3.5 space-y-2.5">
          {card.labels.length > 0 && (
            <div className="flex flex-wrap gap-1">
              {card.labels.map((label) => (
                <span
                  key={label.id}
                  className={`text-[11px] font-semibold px-2 py-0.5 rounded-md ${labelColorMap[label.color]}`}
                >
                  {label.name}
                </span>
              ))}
          ...
          <div className="flex items-center gap-3 flex-wrap">
            {(card.startDate || card.dueDate) && (
              <span
                className={`inline-flex items-center gap-1 text-[11px] font-medium ${card.dueDate && new Date(card.dueDate) < new Date() ? 'text-red-500' : 'text-gray-400 dark:text-gray-500'}`}
              >
            ...
            {card.assignees[0] && (
              <div className="flex -space-x-1.5">
                {card.assignees.slice(0, 3).map((a) => (
                  <div
                    key={a.id}
                    className={`w-6 h-6 rounded-full flex items-center justify-center text-white text-[10px] font-medium ring-2 ring-white dark:ring-gray-800 ${a.avatar === 'JD' ? 'bg-blue-500' : a.avatar === 'AR' ? 'bg-purple-500' : a.avatar === 'MK' ? 'bg-orange-500' : 'bg-green-500'}`}
                  >
                    {a.avatar}
                  </div>
                ))}
