          ...
          {boards.map((board) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-colors ${
                currentBoardId === board.id ? 'bg-gray-700/50 text-white' : 'text-gray-300 hover:bg-gray-800/50'
              }`}
            >
              <div
                className={`w-4 h-4 rounded ${
                  board.id === 'board-1' ? 'bg-blue-500' : board.id === 'board-2' ? 'bg-purple-500' : 'bg-orange-500'
                }`}
              />
