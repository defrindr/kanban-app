  ...
  return (
    <header className="h-14 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between px-4 lg:px-5 flex-shrink-0">
      <div className="flex items-center gap-3 min-w-0">
        ...
        <h1 className="text-base lg:text-lg font-semibold text-gray-900 dark:text-white truncate max-w-[120px] lg:max-w-[200px]">
          {boardName}
        </h1>
        ...
        <div className="hidden sm:flex items-center gap-1.5 ml-2">
          <div className="flex -space-x-2">
            {members.slice(0, 3).map((m) => (
              <div
                key={m.id}
                className="w-7 h-7 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center text-white text-[10px] font-medium border-2 border-white dark:border-gray-900"
                title={m.name}
              >
                {m.avatar}
              </div>
            ...
            {members.length > 3 && (
              <button className="w-7 h-7 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-gray-500 dark:text-gray-400 text-[10px] font-medium border-2 border-white dark:border-gray-900 hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">
                +{members.length - 3}
              </button>
            )}
          </div>
      ...
      <div className="flex items-center gap-1">
        ...
        <button
          ...
          className={`p-2 rounded-lg transition-colors ${showArchived ? 'bg-amber-100 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800'}`}
                ...
                <div className="overflow-y-auto flex-1">
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((n) => (
                      <div
                        key={n.id}
                        className={`flex items-start gap-3 px-4 py-3 border-b border-gray-50 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors ${!n.read ? 'bg-blue-50/50 dark:bg-blue-900/10' : ''}`}
                      >
                        <div
                          className={`mt-1.5 w-2 h-2 rounded-full flex-shrink-0 ${!n.read ? 'bg-blue-500' : 'bg-transparent'}`}
                        />
