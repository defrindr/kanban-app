...
describe('KanbanHeader', () => {
  ...
  it('renders member avatar', () => {
    render(
      <KanbanHeader {...base} members={[{ id: 'u1', name: 'John', email: 'j@t.com', role: 'MEMBER', avatar: 'JD' }]} />
    );
  ...
  it('shows clear search button', () => {
    ...
    for (const b of buttons) {
      if (b.querySelector('svg path[d*="M6 18L18 6"]')) {
        fireEvent.click(b);
        break;
      }
    }
  ...
  it('shows notification unread count', () => {
    ...
    for (const b of notifBtns) {
      if (b.querySelector('svg path[d*="14.857"]')) {
        fireEvent.click(b);
        break;
      }
    }
