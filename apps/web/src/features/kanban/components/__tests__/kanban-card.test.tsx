...
describe('KanbanCard', () => {
  ...
  it('shows comment count', () => {
    const card = makeCard({
      comments: [
        { id: 'cm1', cardId: 'c1', userId: 'u1', userName: 'John', content: 'Hi', createdAt: '2026-01-01T00:00:00Z' },
      ],
    });
    render(<KanbanCard card={card} onClick={vi.fn()} />);
  ...
  it('shows checklist progress', () => {
    const card = makeCard({
      checklist: [
        { id: 'ch1', text: 'x', done: true },
        { id: 'ch2', text: 'y', done: false },
      ],
    });
    render(<KanbanCard card={card} onClick={vi.fn()} />);
  ...
  it('shows attachment count', () => {
    const card = makeCard({
      attachments: [
        { id: 'att1', name: 'file.pdf', url: '/u/file.pdf', type: 'pdf', createdAt: '2026-01-01T00:00:00Z' },
      ],
    });
    render(<KanbanCard card={card} onClick={vi.fn()} />);
    expect(screen.getByText('1')).toBeInTheDocument();
