...
export const useBoardStore = create<BoardStore>((set, get) => ({
  ...
  updateList: (list) =>
    set((s) => {
      ...
      return {
        currentBoard: {
          ...s.currentBoard,
          lists: s.currentBoard.lists.map((l) => (l.id === list.id ? { ...l, ...list } : l)),
        },
      };
    }),
  removeList: (listId) =>
  ...
  addCard: (card) =>
    set((s) => {
      ...
      return {
        currentBoard: {
          ...s.currentBoard,
          lists: s.currentBoard.lists.map((l) => (l.id === card.listId ? { ...l, cards: [...l.cards, card] } : l)),
        },
      };
    }),
  updateCard: (card) =>
  ...
  moveCard: (card, fromListId, toListId, newPosition) =>
    set((s) => {
      if (!s.currentBoard) return s;
  ...
  removeCard: (cardId) =>
    set((s) => {
      ...
      return {
        currentBoard: {
          ...s.currentBoard,
          lists: s.currentBoard.lists.map((l) => ({ ...l, cards: l.cards.filter((c) => c.id !== cardId) })),
        },
      };
    }),
  ...
  markNotificationRead: (nid) =>
    set((s) => ({ notifications: s.notifications.map((n) => (n.id === nid ? { ...n, read: true } : n)) })),
        ...
        currentBoard: {
          ...
          lists: s.currentBoard.lists.map((l) => ({
            ...
            cards: l.cards.map((c) => {
              ...
              return exists !== -1
                ? { ...c, labels: c.labels.filter((_, i) => i !== exists) }
  ...
  toggleChecklistItem: (cardId, itemId, done) =>
    set((s) => {
      if (!s.currentBoard) return s;
      return {
        currentBoard: {
          ...
          lists: s.currentBoard.lists.map((l) => ({
            ...
            cards: l.cards.map((c) =>
              c.id === cardId
                ? { ...c, checklist: (c.checklist || []).map((ci) => (ci.id === itemId ? { ...ci, done } : ci)) }
                : c
            ),
          })),
        },
          ...
          lists: s.currentBoard.lists.map((l) => ({
            ...
            cards: l.cards.map((c) =>
              c.id === cardId
                ? {
                    ...c,
                    assignees: c.assignees.some((a) => a.id === member.id) ? c.assignees : [...c.assignees, member],
                  }
                : c
            ),
          })),
  ...
  removeAssignee: (cardId, memberId) =>
    set((s) => {
      if (!s.currentBoard) return s;
      return {
        currentBoard: {
          ...
          lists: s.currentBoard.lists.map((l) => ({
            ...l,
            cards: l.cards.map((c) =>
              c.id === cardId ? { ...c, assignees: c.assignees.filter((a) => a.id !== memberId) } : c
            ),
          })),
        },
      };
