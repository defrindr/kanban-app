...
function makeCard(overrides?: Partial<Card>): Card {
  return {
    id: 'c1', listId: 'l1', title: 'Task', description: '', position: 1,
    labels: [], assignees: [], comments: [], checklist: [], attachments: [],
...
function makeBoard(overrides?: any) {
  return {
    id: 'b1', name: 'Board', description: '', ownerId: 'u1',
    lists: [
      { id: 'l1', boardId: 'b1', title: 'To Do', position: 1, cards: [makeCard(), makeCard({ id: 'c2', title: 'Task 2', position: 2 })] },
...
beforeEach(() => {
  useBoardStore.setState({ boards: [], currentBoard: null, activities: [], notifications: [], loading: false, error: null })
})
...
describe('BoardStore edge cases', () => {
  ...
  it('addComment only adds to correct card', () => {
    useBoardStore.getState().setCurrentBoard(makeBoard())
    useBoardStore.getState().addComment('c1', { id: 'cm1', cardId: 'c1', userId: 'u1', userName: 'John', content: 'Hello', createdAt: '2026-01-01T00:00:00Z' })
    const c1 = useBoardStore.getState().currentBoard?.lists.find(l => l.id === 'l1')?.cards.find(c => c.id === 'c1')
    const c2 = useBoardStore.getState().currentBoard?.lists.find(l => l.id === 'l1')?.cards.find(c => c.id === 'c2')
