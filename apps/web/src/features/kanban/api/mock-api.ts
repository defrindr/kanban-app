...
export async function fetchBoard(
  boardId: string
): Promise<{ ok: true; data: Board } | { ok: false; error: { code: string; message: string } }> {
...
export async function createBoard(
  ...
  if (!res.ok)
    return {
      ok: true,
      data: { id: '', name, description, ownerId: '', lists: [], members: [], createdAt: '', updatedAt: '' },
    };

  const board = transformBoard(res.data);

  if (template && template !== 'Blank') {
    const templateLists: string[] =
...
export async function updateList(
...
): Promise<{ ok: true; data: List } | { ok: false; error: { code: string; message: string } }> {
  ...
  if (!b.ok)
    return { ok: true, data: { id: listId, boardId: '', title: '', position: data.position || 0, cards: [] } as List };
  const list = b.data.lists.find((l) => l.id === listId);
  return {
    ok: true,
    data: list || ({ id: listId, boardId: '', title: '', position: data.position || 0, cards: [] } as List),
  };
}
...
): Promise<{ ok: true; data: Card } | { ok: false; error: { code: string; message: string } }> {
...
export async function moveCard(
  ...
  toListId: string,
  newPosition: number
): Promise<{ ok: true; data: { id: string; listId: string; position: number; updatedAt: string } }> {
  ...
  return {
    ok: true,
    data: { id: cardId, listId: toListId, position: newPosition, updatedAt: new Date().toISOString() },
  };
}
...
export async function uploadAttachment(
  cardId: string,
  file: File
): Promise<{ ok: true; data: Record<string, unknown> } | { ok: false; error: { code: string; message: string } }> {
...
export async function updateComment(
  commentId: string,
  content: string
): Promise<{ ok: true; data: Comment } | { ok: false; error: { code: string; message: string } }> {
...
export async function getCommentsByCard(
  cardId: string,
  page: number = 1,
  limit: number = 50
): Promise<{ ok: true; data: Comment[] }> {
...
export async function updateMemberRole(
  ...
  memberId: string,
  role: 'MEMBER' | 'ADMIN'
): Promise<{ ok: true; data: BoardMember } | { ok: false; error: { code: string; message: string } }> {
...
export async function createWebhook(
  ...
  url: string,
  events: string[]
): Promise<{ ok: true; data: any } | { ok: false; error: { code: string; message: string } }> {
  const res = await apiClient<any>(`/api/boards/${boardId}/webhooks`, {
    method: 'POST',
