...
const BASE_BOARD = {
  ...
  ownerId: 'u1',
  lists: [],
  members: [{ user: { id: 'u1', name: 'John', email: 'john@test.com' } }],
  createdAt: '2026-01-01T00:00:00Z',
  updatedAt: '2026-01-01T00:00:00Z',
};
...
describe('mock-api', () => {
  ...
  describe('fetchBoard', () => {
    it('fetches board and cards', async () => {
      mockApiClient.mockResolvedValueOnce({ ok: true, data: BASE_BOARD }).mockResolvedValueOnce({ ok: true, data: [] });
  ...
  describe('upload and activity', () => {
    it('uploads attachment', async () => {
      global.fetch = vi.fn().mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ data: { id: 'att1' } }),
      });
