...
export function registerSocketHandlers(io: Server) {
  io.on('connection', (socket: Socket) => {
    ...
    if (user) void socket.join(`user:${user.userId}`);

    socket.on(
      'board:join',
      guarded(socket, 'board:join', async (raw: unknown) => {
    ...
    socket.on(
      'board:leave',
      guarded(socket, 'board:leave', async (raw: unknown) => {
    ...
    socket.on(
      'list:create',
      guarded(socket, 'list:create', async (raw: unknown) => {
    ...
    socket.on(
      'list:update',
      guarded(socket, 'list:update', async (raw: unknown) => {
    ...
    socket.on(
      'list:delete',
      guarded(socket, 'list:delete', async (raw: unknown) => {
    ...
    socket.on(
      'card:create',
      guarded(socket, 'card:create', async (raw: unknown) => {
    ...
    socket.on(
      'card:update',
      guarded(socket, 'card:update', async (raw: unknown) => {
    ...
    socket.on(
      'card:move',
      guarded(socket, 'card:move', async (raw: unknown) => {
        ...
        try {
          ...
          if (list) {
            io.to(`board:${list.boardId}`).emit('card:moved', {
              card,
    ...
    socket.on(
      'card:delete',
      guarded(socket, 'card:delete', async (raw: unknown) => {
    ...
    socket.on(
      'comment:add',
      guarded(socket, 'comment:add', async (raw: unknown) => {
